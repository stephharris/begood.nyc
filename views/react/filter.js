  ...
  displayCategories(categories){
    return categories ? categories.map((category, i) => {
      return (
          ...
          <h3 tableindex="0" role="menuitem" className={ category.selected ? 'filterSelected' : 'filterDefault'}>{category.name}</h3>
          </div>
      )
    }) : '';
  }
